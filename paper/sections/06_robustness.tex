...
Sign-based results are far more robust to the splice than magnitudes, since a sign
does not depend on the level convention of a vintage. The persistence results of
Section~\ref{sec:results} therefore carry more weight than the means beside them,
and we have kept the two visually separate for that reason.
...
and a selected date reads as more determined than it is. Two guards are reported.
...
\input{generated/tab_breakstability}
...
The balance does not identify a mechanism. Nothing in this paper separates
cyclical from discretionary movements, and no cyclical adjustment is attempted. A
specification relating balances to nominal GDP growth is available in the
repository and is reported there in an appendix, where its weaknesses are set out;
it is deliberately not part of this paper's evidence, because its dependent variable
and its regressor share a construction through nominal GDP and the fits are very
low. A better-founded mechanism for the Social Security side would model the
contribution base directly, regressing the change in contributions on the change in
the aggregate wage bill $W_t = N_t \bar{w}_t$ rather than on aggregate nominal
growth. We flag it as the natural next step and do not implement it here.
