...
\HistoricalPrimaryPositive\ of \HistoricalPrimaryYears\ over \HistoricalPrimaryWindow\
and \ModernPrimaryPositive\ of \ModernPrimaryYears\ over \ModernPrimaryWindow, so the
positive years are if anything an earlier phenomenon than a recent one. A persistently
negative B.9 does not imply a persistently negative primary balance, and interest is the
arithmetic that separates them: it averaged \HistoricalInterestMean\% of GDP over the
first window against \ModernInterestMean\% over the second.
