...
\input{generated/tab_sources}

\subsection{Vintage}

A fiscal series has a vintage as well as a coverage, and the two are different
things. The \SourceCount\ bundled files do not share a vintage: they range from
\VintageEarliest\ to \VintageLatest, and Table~\ref{tab:sources} records each one
separately for that reason. Within the modern files, \ProvisionalYears\ are flagged
\textbf{provisional} by the publisher, and that flag is carried through the
harmonisation into the canonical panel rather than dropped.
...
as a statement about the vintage recorded for the file it came from.
...
\input{generated/tab_validation}
