...
natural base is the aggregate wage bill \(W_t = N_t \bar{w}_t\), with \(N\) employees
and \(\bar{w}\) the average wage. Writing the effective ratio as \(\tau_t = C_t / W_t\),
...
INE. \(\tau\) is \emph{not} a statutory contribution rate: national-accounts
...
\begin{table}[htbp]
\centering
\caption{The annual change in Social Security contributions, split into the movement of the wage bill and the movement of the effective ratio between them.}
\label{tab:contributionbase}
\small
\fitwidth{%
\begin{tabular}{@{}rrrrr@{}}
\toprule
Year & Change in contributions (M EUR) & From the wage bill (M EUR) & From the effective rate (M EUR) & Interaction (M EUR) \\
\midrule
...
\par\smallskip
\begin{minipage}{\linewidth}\footnotesize\itshape The interaction term is carried rather than dropped or shared between the two effects, because either would make the decomposition inexact. It closes to 3.2e-12 M EUR.\end{minipage}
\end{table}
...
\begin{table}[htbp]
\centering
\caption{And what moved the wage bill: the number of employees against the average wage per employee.}
...
\caption{Contributions and their base. Upper panel: the change in contributions split into a base effect and a rate effect. Lower panel: the change in the wage bill split into employment and average wages.}
...
The slope sits close to the mean effective ratio of 0.221, which is what the
accounting predicts, and the fit is far tighter than the nominal-GDP specification
in Appendix~\ref{sec:comovement}: the regressor here is the base the levy actually
