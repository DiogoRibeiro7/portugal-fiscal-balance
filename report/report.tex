...
\par\smallskip
\begin{minipage}{\linewidth}\footnotesize\itshape The two source families resolve the accounts at different depths: the modern workbooks separate four revenue and seven expenditure components, the historical series only current from capital. Each sector-year uses the finer scheme it reports, so the modern period is never coarsened to match the historical one. Expenditure contributions are negated, so a falling capital expenditure appears as a positive contribution.\end{minipage}
\end{table}
...
wages, so the aggregate employee wage bill is their natural reference base, \(W_t = N_t \bar{w}_t\), with \(N\) employees
and \(\bar{w}\) the average wage. It is a reference base and not the integral base of the
levy: some recorded contributions fall on other bases and some are imputed, so \(W\)
does not exhaust what \(C\) is charged on. Writing the contributions-to-wage-bill
ratio as \(\tau_t = C_t / W_t\),
...
INE. \(\tau\) is \emph{not} a statutory contribution rate: its numerator contains revenue
...
stands at \textbf{0.265} in 2025 against \textbf{0.260} in 1995.

\begin{table}[htbp]
\centering
\caption{The annual change in Social Security contributions, split into the movement of the wage bill, the movement of the contributions-to-wage-bill ratio, and their interaction.}
...
\par\smallskip
\begin{minipage}{\linewidth}\footnotesize\itshape The interaction term is carried rather than dropped or shared between the two factors, because either would make the decomposition inexact. It closes to 3.2e-12 M EUR.\end{minipage}
\end{table}
In 2025 contributions rose by \textbf{2,468 M EUR},
of which \textbf{1,870 M EUR} came from
the wage bill and \textbf{560 M EUR} from the
...
\begin{table}[htbp]
\centering
\caption{And what moved the wage bill itself: the number of employees against the average wage per employee. This decomposes the change in the wage bill, not the wage-bill component of the change in contributions; the two have different totals.}
...
\label{tab:contributionsymmetric}
\small
\fitwidth{%
\begin{tabular}{@{}rrrrr@{}}
\toprule
...
\par\smallskip
\begin{minipage}{\linewidth}\footnotesize\itshape Exact in two terms rather than three. Neither convention is more correct; the three-term form is reported above because carrying the interaction rather than distributing it is the more conservative presentation.\end{minipage}
\end{table}
\begin{figure}[htbp]
\centering
\includegraphics[width=1\linewidth]{19_contribution_base.png}
\caption{Contributions and their reference base. Upper panel: the change in contributions split into a wage-bill component, a ratio component and their interaction. Lower panel: the change in the wage bill split into an employment component, an average-wage component and their interaction.}
\label{fig:contributionbasefig}
\end{figure}
\subsection{A companion regression}

Regressing the annual change in contributions on the annual change in the wage
bill over 25 adjacent-year pairs gives a slope of
\textbf{0.248} with a HAC standard error of
0.022 and an \(R^2\) of \textbf{0.926}.
The slope sits close to the mean contributions-to-wage-bill ratio of 0.221, which is
...
in Appendix~\ref{sec:comovement}: the regressor here is a quantity the levy is
...
\begin{table}[htbp]
\centering
\caption{The same frequencies inside each statistical regime. The counts may be pooled; the interest magnitudes may not.}
\label{tab:primarysignsregime}
\small
\fitwidth{%
\begin{tabular}{@{}llrrrrr@{}}
\toprule
Regime & Sector & N & Headline < 0 & Primary > 0 & Mean interest (\% GDP) & Peak interest (\% GDP) \\
\midrule
1977--1994 historical & General Government & 18 & 18 & 9 & 5.66 & 7.89 \\
1995--2025 modern & General Government & 31 & 27 & 13 & 3.29 & 5.54 \\
1977--1994 historical & Central Government & 19 & 19 & 9 & 5.52 & 7.73 \\
1995--2025 modern & Central Government & 26 & 26 & 6 & 3.19 & 4.88 \\
1977--1994 historical & Regional and Local & 19 & 13 & 10 & 0.18 & 0.62 \\
1995--2025 modern & Regional and Local & 26 & 16 & 10 & 0.09 & 0.18 \\
1977--1994 historical & Social Security Funds & 19 & 5 & 14 & 0.01 & 0.04 \\
1995--2025 modern & Social Security Funds & 26 & 1 & 25 & 0.00 & 0.00 \\
\bottomrule
\end{tabular}%
}
\par\smallskip
\begin{minipage}{\linewidth}\footnotesize\itshape Interest is a magnitude and the two regimes differ materially in it, so the pooled means of Table~\ref{tab:primarysigns} describe the historical regime better than the modern one. The sign counts are reported pooled and split because the classification survives the splice where the magnitudes do not.\end{minipage}
\end{table}
...
This leaves \textbf{28 reporters} over
1995--2025. Eurostat's
...
\begin{table}[htbp]
\centering
\caption{Subsector composition across European reporters, ordered by the frequency of a Social Security surplus.}
\label{tab:benchmark}
\small
\fitwidth{%
\begin{tabular}{@{}lrrrrrrr@{}}
...
\par\smallskip
\begin{minipage}{\linewidth}\footnotesize\itshape The non-Social-Security aggregate is central plus state plus local government, so federal reporters are treated consistently with unitary ones. Reporters with fewer than fifteen complete years are excluded.\end{minipage}
\end{table}
...
\textbf{100\%} of Portuguese years, but
\textbf{9 of 28 reporters} record one in every year they
...
\textbf{93.5\%} of Portuguese years against a
cross-country median of
\textbf{68.9\%}, and averaging
\textbf{0.71\% of GDP} against a median of
\textbf{0.17\%}.

\begin{figure}[htbp]
\centering
\includegraphics[width=1\linewidth]{17_european_benchmark.png}
\caption{Subsector sign frequencies across reporters, Portugal highlighted and reporters ordered by the frequency of a Social Security surplus.}
\label{fig:benchmarkfig}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=1\linewidth]{18_european_offset_distribution.png}
\caption{Offset ratio where defined: Portugal against all other reporters pooled, as a share of each group's defined country-years. Values above three are clipped.}
\label{fig:benchmarkoffset}
\end{figure}

The sharpest comparison is the composition of a surplus year. Across the
\textbf{22 reporters} with at least one aggregate surplus, a negative
non-Social-Security balance accompanies the surplus in
\textbf{54 of 198} surplus country-years, or
\textbf{27\%}. It is a minority pattern.
\textbf{1 of those reporters} show it in every one of their surplus
years, Portugal among them, on
\textbf{4 surplus years} --- a count small enough
...
Contribution base & \texttt{09\_social\_security\_mechanisms} & \texttt{outputs/tables/contribution\_change\_decomposition.csv} \\
European benchmark & \texttt{16\_european\_benchmark} & \texttt{outputs/tables/european\_benchmark\_summary.csv} \\
