...
def primary_balance_sign_summary_by_regime(primary: pd.DataFrame) -> pd.DataFrame:
    """Split the sign summary by statistical regime.

    The pooled version averages interest across the 1995 splice, and the two regimes
    differ enough that the pooled figure describes neither: interest is far heavier in
    the historical period. The same discipline the balance magnitudes get is applied
    here, for the same reason.
    """
    records: list[dict[str, float | int | str]] = []
    for (sector, regime), group in primary.groupby(["sector", "statistical_regime"], sort=False):
        recomputed = group["primary_balance_recomputed_m_eur"]
        positive_years = sorted(int(year) for year in group.loc[recomputed > 0, "year"])
        records.append(
            {
                "sector": str(sector),
                "statistical_regime": str(regime),
                "first_year": int(group["year"].min()),
                "last_year": int(group["year"].max()),
                "n_years": int(len(group)),
                "headline_negative_years": int((group["balance_m_eur"] < 0).sum()),
                "primary_positive_years": int((recomputed > 0).sum()),
                "primary_positive_year_list": ";".join(str(year) for year in positive_years),
                "mean_interest_pct_gdp": float(group["interest_pct_gdp"].mean()),
                "max_interest_pct_gdp": float(group["interest_pct_gdp"].max()),
